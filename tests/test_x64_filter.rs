...
fn test_x64_filter_bad_bytes() {
    ...
    let good_bytes_gadgets = xgadget::filter_bad_addr_bytes(gadgets, &[0x10, 0x14, 0x15, 0xc, 0xd]);
...
#[test]
fn test_x64_filter_reg_no_deref_1() {
    let bin_filters = common::get_raw_bin("bin_filters", common::FILTERS_X64);
    let bins = vec![bin_filters];
    let gadgets =
        xgadget::find_gadgets(&bins, common::MAX_LEN, xgadget::SearchConfig::default()).unwrap();
    let reg_no_deref_gadgets = xgadget::filter_reg_no_deref(gadgets, None);
    let reg_no_deref_gadget_strs = common::get_gadget_strs(&reg_no_deref_gadgets, false);
    common::print_gadget_strs(&reg_no_deref_gadget_strs);

    // Positive --------------------------------------------------------------------------------------------------------
    assert!(common::gadget_strs_contains_sub_str(
        &reg_no_deref_gadget_strs,
        "pop r8; ret;"
    ));
    assert!(common::gadget_strs_contains_sub_str(
        &reg_no_deref_gadget_strs,
...
#[test]
fn test_x64_filter_reg_no_deref_2() {
    let bin_filters = common::get_raw_bin("bin_filters", common::FILTERS_X64);
    let bins = vec![bin_filters];
    let gadgets =
        xgadget::find_gadgets(&bins, common::MAX_LEN, xgadget::SearchConfig::default()).unwrap();
    let reg_no_deref_gadgets =
        xgadget::filter_reg_no_deref(gadgets, Some(&[iced_x86::Register::RCX]));
    let reg_no_deref_gadget_strs = common::get_gadget_strs(&reg_no_deref_gadgets, false);
    common::print_gadget_strs(&reg_no_deref_gadget_strs);
