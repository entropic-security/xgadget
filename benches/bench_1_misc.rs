...
fn opcode_offset_bench(c: &mut Criterion) {
    ...
    let search_config = xgadget::SearchConfig::ROP | xgadget::SearchConfig::JOP | xgadget::SearchConfig::IMM16;
    ...
    c.bench_function("off_seq_base_1_000", |b| b.iter(|| find_match_idxs_sequential_baseline(&rand_1_000, &op_codes)));
    c.bench_function("off_seq_func_1_000", |b| b.iter(|| find_match_idxs_sequential_baseline(&rand_1_000, &op_codes)));
    c.bench_function("off_par_bin_1_000", |b| b.iter(|| seg.get_matching_offsets(&op_codes)));

    let bin = xgadget::Binary::from_bytes("test_10_000", &rand_10_000).unwrap();
    let seg = bin.segments.iter().next().unwrap();
    c.bench_function("off_seq_base_10_000", |b| b.iter(|| find_match_idxs_sequential_baseline(&rand_10_000, &op_codes)));
    c.bench_function("off_seq_func_10_000", |b| b.iter(|| find_match_idxs_sequential_baseline(&rand_10_000, &op_codes)));
    c.bench_function("off_par_bin_10_000", |b| b.iter(|| seg.get_matching_offsets(&op_codes)));

    let bin = xgadget::Binary::from_bytes("test_100_000", &rand_100_000).unwrap();
    let seg = bin.segments.iter().next().unwrap();
    c.bench_function("off_seq_base_100_000", |b| b.iter(|| find_match_idxs_sequential_baseline(&rand_100_000, &op_codes)));
    c.bench_function("off_seq_func_100_000", |b| b.iter(|| find_match_idxs_sequential_baseline(&rand_100_000, &op_codes)));
    c.bench_function("off_par_bin_100_000", |b| b.iter(|| seg.get_matching_offsets(&op_codes)));
}

// Stack Pivot Filter --------------------------------------------------------------------------------------------------

pub fn filter_stack_pivot_sequential<'a>(gadgets: &Vec<xgadget::gadget::Gadget<'a>>) -> Vec<xgadget::gadget::Gadget<'a>> {
    gadgets.iter()
        .filter(|g| {
            for i in &g.instrs {
                for o in &i.operands {
...
fn pivot_bench(c: &mut Criterion) {
    ...
    let readelf_gadgets = xgadget::find_gadgets(&bins, MAX_GADGET_LEN, xgadget::gadget::SearchConfig::DEFAULT).unwrap();
